...
static Pair computeCPair(int i, int j, int n){
  int pos, loss;
  loss = (n*i) - ((i*(i+1))/2);
  pos = ((i * n) + j) - loss;
  Pair toRet;
...
void print_array(int n,
		 DATA_TYPE POLYBENCH_2D(Cn,N,N,n,n)){
  int i, j;
  Pair temp;
  ...
  for (i = 0; i < n; i++)
    for (j = 0; j <= i; j++){
      //
      // Changed to print upper triangular part above.
      //
	if ((i * n + j) % 20 == 0) fprintf (POLYBENCH_DUMP_TARGET, "\n");
        temp = computeCPair(i,j,n);
	fprintf (POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, Cn[temp.val1][temp.val2]);
    }
...
void kernel_syr2k(int n, int m,
  ...
  */
  ...
  // Temporary pair variable
  Pair temp;
  int cJump = 100; // Constant for now.
...
#pragma omp parallel for private(temp)
  for (ii=0; ii < _PB_N; ii += cJump){
    for (i = ii; i < fmin(ii + cJump, _PB_N); i++) {
      for (j = 0; j <= i; j++){
        temp = computeCPair(i,j,n);
        Cn[temp.val1][temp.val2] = C[i][j];
      }
    }
  }
...
#pragma omp parallel for private(temp)
  for (i = 0; i < _PB_N; i++) {
    for (j = 0; j <= i; j++){
      temp = computeCPair(i,j,n);
      Cn[temp.val1][temp.val2] *= beta;
      for (k = 0; k < _PB_M; k++){
        Cn[temp.val1][temp.val2] += A[j][k]*alpha*B[i][k] + B[j][k]*alpha*A[i][k];
      }
    }
  }
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_2D_ARRAY_DECL(Cn,DATA_TYPE,N,N,n,n);
  ...
  polybench_prevent_dce(print_array(n, POLYBENCH_ARRAY(Cn)));

  /* Be clean. */
  POLYBENCH_FREE_ARRAY(C);
  POLYBENCH_FREE_ARRAY(Cn);
