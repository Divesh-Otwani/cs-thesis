		  ...
		  DATA_TYPE POLYBENCH_2D(Bt,M,N,m,n))
{
  ...
  */
  ...
  //int jumpB = floor((cacheSize * 1024 ) / (18*8*4) );
  int jumpB = 64;
  int jump = jumpA;
...
#pragma omp parallel for private(ii,jj,i,j)
...
#pragma omp parallel for private(ii,jj,i,j)
  for (ii=0; ii < _PB_N; ii += jumpB){
    for(jj=0; jj <= i; jj += jumpB){
      for (i=ii; i < fmin(jumpB + ii, _PB_N); i++){
        for (j=jj; j <= fmin(jumpB + jj, i); j++){
          C[i][j] *= beta;
        }
      }
    }
...
#pragma omp parallel for private(i,kk,k,j)
  for (i = 0; i < _PB_N; i++) {
    for(kk = 0; kk < _PB_M; kk += jumpB){
      for(k = kk; k < fmin(_PB_M, kk+jumpB); k++){
