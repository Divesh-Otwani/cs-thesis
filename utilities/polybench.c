...
# if defined(POLYBENCH_TIME) || defined(POLYBENCH_GFLOPS)
void polybench_set_program_flops(double total_fp_ops_for_kernel)
{
	polybench_program_total_flops = total_fp_ops_for_kernel;
}
#endif
...
# pragma omp parallel
  {
	...
	for (evid = 0; polybench_papi_eventlist[evid] != 0; ++evid)
	  {
	    if (verbose)
	      printf ("%-12s=", _polybench_papi_eventlist[evid]);
